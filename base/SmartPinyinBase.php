...
class SmartPinyinBase
{
    ...
    public function valueSplitCnPinyin()
    {
        ...
        foreach($valueArr as $k => $v){
            if($k == 0){
                $this->_value_splited[$i] = $v;
            }else{
                $prev = empty($this->_value_splited[$i])? '' : $this->_value_splited[$i];
                if(
                    $v == ' ' ||
                ...
                if($v != ' '){
                    if(!isset($this->_value_splited[$i])){
                        $this->_value_splited[$i] = '';
                    }
                    $this->_value_splited[$i] .= $v;
                }
            }
        ...
        foreach($this->_glues as $glue){
            ...
            foreach($this->_value_splited as $k => $char){
                ...
                if(!empty($char)){
                    ...
                    $charPinyin[$k_char] = array_merge($charPinyin[$k_char], $pinyins);
                    if(!self::IsAllChinese($char)){
                        foreach($pinyins as $pinyin){
                            $pinyinAnalyzers = self::PinyinAnalyzer($pinyin);
                            $this->pushChar($pinyinAnalyzers['pinyins']);
                            foreach($pinyinAnalyzers['pinyins'] as $analyzedPinyin){
                                $singleCombo = self::SinglePinYinComb($analyzedPinyin, $glue, $this->_dynamic_glue, $this->_single_ymchar_split);
            ...
            foreach($charPinyin as $char => $pinyin){
                $charPinyin[$char] = array_unique($pinyin);
                if(empty($charPinyin[$char])){
                    $charPinyin[$char] = array($char);
                }
            }
            
            $combs = self::CharPinYinsComb($charPinyin, $glue, $this->_dynamic_glue, $this->_single_ymchar_split);
     ...
     */
    public static function CharPinYinsComb($charPinyins, $glues = ' ', $dynamicGlue = false, $singleYmCharSplit = true, $deep = 0)
    {
        $ret = [];
        ...
        if(is_string($glues)){
            ...
            foreach($firstCharPinyins as $pinyin){
                ...
                if(count($remainCharPinyins) == 1){
                    ...
                    foreach(self::CharPinYinsComb($remainCharPinyins, $glues, $dynamicGlue, $singleYmCharSplit, $deep+1) as $remainPinyin){
                        $ret[] = $pinyin . $glue . $remainPinyin;
                        if($dynamicGlue){
                            $ret[] = $pinyin . $remainPinyin;
                        }
            ...
            foreach($glues as $glue){
                foreach(self::CharPinYinsComb($charPinyins, $glue, $dynamicGlue, $singleYmCharSplit, $deep) as $remainPinyin){
                    $ret[] = $remainPinyin;
                }
            }
        }
     ...
     */
    public static function SinglePinYinComb($singlePinyin, $glues = ' ', $dynamicGlue = false, $singleYmCharSplit = true, $deep = 0)
    {
        $ret = [];
        ...
        if(empty($remainSinglePinyin)){
            ...
            if(is_string($glues)){
                ...
                if(count($remainSinglePinyin) <= 1){
                    ...
                    foreach(self::SinglePinYinComb(implode(' ', $remainSinglePinyin), $glues, $dynamicGlue, $singleYmCharSplit, $deep+1) as $remainPinyin){
                ...
                foreach($glues as $glue){
                    foreach(self::SinglePinYinComb($singlePinyin, $glue, $dynamicGlue, $singleYmCharSplit, $deep) as $remainPinyin){
                        $ret[] = $remainPinyin;
                    }
                }
            }
    ...
    public static function PinyinAnalyzer($str)
    {
        $str = strtolower($str);
        $pinyins = [
            'chars' => [],
        ...
        if(count($splitBySpace) > 1){
            foreach($splitBySpace as $splitStr){
                $recrRet = self::PinyinAnalyzer($splitStr);
